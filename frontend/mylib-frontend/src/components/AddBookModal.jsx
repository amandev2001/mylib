...
export default function AddBookModal({ isOpen, onClose, onSubmit }) {
  ...
  const handleSubmit = async (formData) => {
    try {
      ...
      setSubmitSuccess('Book added successfully!');
    } catch (error) {
              ...
              <Dialog.Panel className={`relative transform overflow-hidden rounded-lg ${isDarkMode ? 'bg-gray-800' : 'bg-white'} px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-2xl sm:p-6`}>
                ...
                <div className="sm:flex sm:items-start">
                  <div className="mt-3 text-center sm:mt-0 sm:text-left w-full">
                    ...
                    {submitSuccess && (
                      <div className={`mb-4 text-sm ${isDarkMode ? 'text-green-400 bg-green-900/50' : 'text-green-600 bg-green-50'} p-3 rounded-md`}>
                        {submitSuccess}
                      </div>
                    )}
